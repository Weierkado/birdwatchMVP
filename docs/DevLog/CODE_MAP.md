...
- `index.html`
  ...
  - `#searchOverlay > #searchMover > #searchGlass` 当前也静态挂在主内容区栈内，由 `src/main.js` 在探索态 ritual delay 时控制显隐、CSS 路径变量与 hard hide；它不是独立 overlay 模块，不接管业务状态，也不参与 LocalStorage。

- `styles/style.css`
  - 主游戏全局样式。
  - 覆盖固定手机壳、HUD / Event Strip / Main Panel / Action Zone、四项底部导航、overlay 壳层、消息面板、笔记文件夹、相册、纸页、卡牌、FOCUS 取景框、拍立得、结算、地图、状态栏事件高亮、RESULT 发妹妹按钮、inline choices、摇杆句柄 / 导引线、搜索 overlay、动画和响应式规则。
...
- 管理探索态 ritual delay 期间的内容区搜索 overlay：`createSearchPlan()` 生成固定 5 个 CSS 路径点与总时长，`startSearchOverlay()` 将路径点写成 `#searchMover` 上的 CSS 变量，`getRitualDelay(action, state, searchPlan)` 在存在搜索计划时直接复用 `searchPlan.totalMs`；移动和 idle 由 `styles/style.css` 的 keyframes 驱动，`syncSearchOverlayWithTransition()` 只做 guard hide，不负责自动重新 start overlay。该层只替换等待表现，不新增 action，也不改 `gameSession.js` 的业务处理顺序。
...
- `activeExploreRitualAction`：探索态搜索 overlay 的当前 ritual action 标记，只用于判断 `turnLeft` / `turnRight` / `observe` 的等待表现是否仍应可见；搜索 overlay 当前不再保留独立 runId、rAF、递归 timer 或 active plan 状态，不进入 `gameState` 或持久化结构。
...
- 搜索 / 放大镜 overlay 当前也属于 `src/main.js` 的 UI 生命周期层；如果继续调等待表现或残留清理，优先检查 `createSearchPlan()`、`startSearchOverlay()`、`hideSearchOverlayNow()`、`resetSearchOverlay()`、`syncSearchOverlayWithTransition()` 和 CSS keyframes，不要把等待、移动或 cleanup 逻辑塞进业务状态机。
...
- `.main-content-stack` 当前必须保持 `position: relative`，供 `#searchOverlay` 在主内容区内绝对定位；`.search-overlay` 只负责整体显隐且不抢 pointer events，`.search-mover` 通过 CSS 变量和 `@keyframes searchMovePath` 控制多段位置移动，`.search-glass` 通过 `@keyframes searchGlassIdle` 控制可见期间的轻微 idle，不应变成全局 fixed 蒙层。
...
- 不要把搜索 overlay 当成观察系统或业务状态机；当前实现是 ritual delay 的 UI-only 等待表现，移动由 CSS keyframes 驱动，JS 只生成路径点、写 CSS 变量、show / hard hide。后续不要重新引入 render 自动 start、rAF、递归 path timer 或多套 active plan 状态来修视觉问题。
...
- 调整探索 ritual 的搜索 / 放大镜 overlay：
  - UI 编排：`src/main.js` 的 `createSearchPlan()`、`startSearchOverlay()`、`stopSearchOverlay()`、`resetSearchOverlay()`、`syncSearchOverlayWithTransition()`
  - 运行时状态：`activeExploreRitualAction` 与 `actionTransitionTimerId`；搜索 overlay 当前不再保留独立 runId、rAF、递归 timer 或 active plan 状态
  - 静态宿主：`index.html` 的 `#searchOverlay`、`#searchMover`、`#searchGlass`
  - 样式节点：`styles/style.css` 的 `.main-content-stack`、`.search-overlay`、`.search-mover`、`.search-glass`、`@keyframes searchMovePath`、`@keyframes searchGlassIdle`
  - 维护边界：只改 ritual delay 的等待表现与 cleanup，不要顺手改 `turnLeft` / `turnRight` / `observe` 的业务分支、`gameSession.js`、LocalStorage、field guide / snapshots 或 Liya queue
...
12. 在 `turnLeft`、`turnRight`、`observe` 的 ritual delay 期间确认主内容区内会出现搜索 / 放大镜 overlay，放大镜沿多段路径移动且 visible 期间持续轻微 idle；动画结束、状态切换、bottom nav 切换或文本内容落地后不应残留在主叙事上。若仍出现残留，优先回归 `src/main.js` 的 show / hard hide 时序和 `styles/style.css` 的 `searchMovePath` / `searchGlassIdle`，不要先动业务状态机。
13. 在 `EXPLORE` 阶段多次转向和远听落地，确认顶部状态栏“周围事件”只在左右侧有可用鸟时出现提示，左右都有鸟时不会固定只出右侧，同鸟点同方向仍受冷却控制，且每次新提示都会重播一次暖色渐隐反馈。
14. 触发 FIRST_ENCOUNTER，确认未提前泄露正式鸟名。
15. 新的一天建立后确认天气先初始化一次；随后点击【开始今天的观鸟】不重新 roll，当天探索进入允许回合窗口后天气仍可最多切换一次，并通过“周围事件”提示短句反馈。
16. 进入 PHOTO，覆盖 DECISION / FOCUS / RESULT / REPOSITION / LOST。
17. 拍摄态点击【再等一等】后，顶部仍保持拍摄态；点击【放弃拍摄】或真正退出 PHOTO 后才回探索态。
18. FOCUS 拍照时确认所见即所得：可见行为状态、对焦位置、缩放、旋转写入 snapshot，且 snapshot 同时保留 `weatherKey`。
19. 成功拍照才消耗电量。
20. RESULT 页首次发送给妹妹后显示 disabled「已发给妹妹」；历史已发送照片再次拍到时不显示按钮，但会补一句「之前也给妹妹发过这张。」。
21. 加新写入 field guide，并记录正确 `cataloguedDayIndex`。
22. 笔记列表、卡牌详情、照片翻页和底部关闭按钮正常。
23. 底部导航显示【观察 / 消息 / 笔记 / 相册】四项；切换消息、笔记、相册时 overlay 互斥，返回观察后 detail panel 收起。
24. 相册空态、列表分页、卡牌级缩略项、详情页、snapshot 翻页和关闭相册正常；相册不应改变笔记鸟种页内容。
25. 相册详情发送妹妹后应与 RESULT / 笔记详情共用已发送状态，不能重复生成 Liya queue。
